...
async function edit() {
  ...
  const renderQuotaOptions = function () {
    ...
    quotaList.innerHTML = visible.length ? visible.map(function (quota) {
      ...
      return `<label class="flex cursor-pointer items-center gap-2 border-b border-slate-100 px-3 py-2 text-sm hover:bg-slate-50"><input type="checkbox" data-library-quota-id="${esc(quota.id)}"${checked} /><span class="min-w-0 flex-1"><span class="font-medium text-slate-800">${esc(quota.name || '未命名定额')}</span><span class="ml-2 text-xs text-teal-700">${esc(quota.code || '未编码')}</span></span><span class="text-xs text-slate-500">${esc(quota.unit || '-')}</span></label>`;
