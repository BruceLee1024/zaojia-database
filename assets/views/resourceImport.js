import { exportResourceTemplate } from '../data/excel.js?v=6.10';
import { resourceImportService } from '../services/resourceImportService.js?v=6.10';
...
async function paint(workspace) {
  ...
  workspace.innerHTML = `<div class="page-frame min-h-full flex flex-col gap-4">
    ...
    <section class="rounded-lg border border-slate-200 bg-white p-5"><input id="resourceImportFile" class="hidden" type="file" accept=".xlsx,.xls" onchange="window.__resourceImport.handle(this)"><div class="border border-dashed border-slate-300 bg-slate-50 px-6 py-8 text-center"><span class="material-symbols-outlined text-4xl text-slate-300">upload_file</span><h2 class="mt-2 font-semibold text-slate-800">${state.fileName ? esc(state.fileName) : `选择${label} Excel`}</h2><p class="mt-1 text-xs text-slate-500">支持 .xlsx / .xls。系统会理解“实际采购”“地区”等业务表达；名称或单位缺失才会暂不写入，异常价格会保留主数据并进入待确认。</p><button onclick="window.__resourceImport.pick()" ${state.busy ? 'disabled' : ''} class="mt-4 h-9 px-4 brand-bg text-white text-sm disabled:opacity-40">${state.busy ? '正在处理…' : '选择文件'}</button></div></section>
    ${preview ? previewMarkup(preview, label) : ''}${state.report ? reportMarkup(state.report) : ''}
  </div>`;
}

function previewMarkup(preview, label) {
  return `<section class="rounded-lg border border-slate-200 bg-white overflow-hidden"><div class="border-b border-slate-200 px-4 py-3 flex flex-wrap items-center gap-3"><div><h2 class="font-semibold text-slate-900">导入预览</h2><p class="mt-1 text-xs text-slate-500">共 ${preview.counts.total} 行，新增 ${preview.counts.create}，更新 ${preview.counts.update}，重复 ${preview.counts.duplicate}，冲突 ${preview.counts.conflict || 0}，主数据无效 ${preview.counts.invalid}，价格待确认 ${preview.counts.priceReview || 0}。</p></div><div class="flex-1"></div><label class="text-xs text-slate-600 flex items-center gap-2"><input id="resourceImportUpdate" type="checkbox" checked>覆盖更新已有${label}</label><button onclick="window.__resourceImport.commit()" ${state.busy || !preview.counts.valid ? 'disabled' : ''} class="h-9 px-4 brand-bg text-white text-sm disabled:opacity-40">确认导入 ${preview.counts.valid} 行</button></div><div class="overflow-auto max-h-[480px]"><table class="w-full text-sm"><thead class="bg-slate-50 text-xs text-slate-500 sticky top-0"><tr><th class="px-3 py-2 text-left">行</th><th class="px-3 text-left">处理</th><th class="px-3 text-left">编码 / 名称</th><th class="px-3 text-left">规格 / 单位</th><th class="px-3 text-right">单价</th><th class="px-3 text-left">检查</th></tr></thead><tbody class="divide-y divide-slate-100">${preview.rows.map(row => `<tr class="${row.errors.length || row.action === 'conflict' ? 'bg-red-50/50' : row.priceReview.length ? 'bg-amber-50/50' : ''}"><td class="px-3 py-2 text-slate-500">${row.index + 1}</td><td class="px-3">${actionBadge(row.action)}</td><td class="px-3"><span class="font-medium text-slate-900">${esc(row.resource.name || '-')}</span><span class="block text-xs text-slate-500">${esc(row.resource.code || '未编码')}</span></td><td class="px-3 text-slate-600">${esc(row.resource.specModel || '-')} · ${esc(row.resource.unit || '-')}</td><td class="px-3 text-right tabular-nums">${row.price ? Number(row.price.unitPrice).toLocaleString('zh-CN') : '-'}</td><td class="px-3 text-xs ${row.errors.length || row.action === 'conflict' ? 'text-red-600' : row.priceReview.length ? 'text-amber-700' : 'text-slate-400'}">${esc(row.errors.join('；') || row.priceReview.join('；') || (row.action === 'conflict' ? '与已有资源冲突' : '通过'))}</td></tr>`).join('')}</tbody></table></div></section>`;
}
...
export function buildImportReportRows(report = {}) {
  return (report.rows || []).map(row => {
    ...
    const priceMessage = row.priceStatus === 'created' ? '价格快照已新增' : row.priceStatus === 'skipped' ? '价格已存在，未重复写入' : row.priceStatus === 'pending' ? '价格待确认，未作为有效价格写入' : '无价格快照';
...
export function buildImportReportViewModel(report = {}) {
  ...
  return {
    ...
    metrics: [
      ...
      { label: '待确认价格', value: counts.pricesPending || 0 },
      { label: '错误', value: counts.errors || 0 },
    ],
  };
}

function actionBadge(action) { const map = { create: ['badge-green', '新增'], update: ['badge-blue', '更新'], duplicate: ['badge-gray', '重复'], conflict: ['badge-red', '冲突'], invalid: ['badge-red', '无效'] }; const [tone, label] = map[action] || map.invalid; return `<span class="badge ${tone}">${label}</span>`; }
