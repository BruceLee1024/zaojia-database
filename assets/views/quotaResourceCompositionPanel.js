...
export async function mountQuotaResourceComposition(quota, { onApplied } = {}) {
  ...
  const render = async () => {
    const [comparisons, resources] = await Promise.all([
      ...
      resourceService.list({ keyword: state.keyword, status: 'active' }),
    ]);
    const selected = resources.find(item => item.id === state.selectedResourceId)
      || (state.selectedResourceId ? await resourceService.get(state.selectedResourceId) : null);
    const prices = selected ? await resourcePriceService.listByResource(selected.id) : [];
    const usages = comparisons.map(item => item.usage);
    const preview = buildCompositionPreview(quota, usages);
    container.innerHTML = panelHtml({ comparisons, resources: resources.slice(0, 8), selected, prices, preview, keyword: state.keyword });
    bindPanel(container, { quota, state, render, preview, onApplied });
  };
...
function panelHtml({ comparisons, resources, selected, prices, preview, keyword }) {
  return `<div class="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
    ...
    <aside><div class="mb-2 flex items-center justify-between"><div class="text-sm font-semibold text-slate-800">组成应用预览</div><button id="quotaCompositionApply" type="button" class="rounded bg-teal-700 px-3 py-1.5 text-xs font-medium text-white hover:bg-teal-800">确认应用</button></div>${renderCompositionPreview(preview)}</aside>
  </div>`;
}
...
function linkForm(resource, prices) {
  return `<div class="rounded border border-teal-200 bg-teal-50 p-3">
    ...
    <div class="grid grid-cols-2 gap-2 xl:grid-cols-[1fr_110px_90px_auto]">
      ...
      <button id="quotaResourceLink" type="button" ${prices.length ? '' : 'disabled'} class="mt-5 h-9 rounded bg-teal-700 px-3 text-sm text-white disabled:cursor-not-allowed disabled:bg-slate-300">关联</button>
...
function comparisonRow(comparison) {
  ...
  return `<div class="rounded border border-slate-200 bg-white p-3">
    ...
    <div class="mt-2 flex justify-end gap-2">${comparison.stale ? `<button type="button" data-refresh-usage="${vm.id}" class="rounded border border-amber-300 px-2 py-1 text-xs text-amber-800 hover:bg-amber-50">刷新快照</button>` : ''}<button type="button" data-remove-usage="${vm.id}" class="rounded border border-red-200 px-2 py-1 text-xs text-red-600 hover:bg-red-50">移除</button></div>
  </div>`;
}
...
function bindPanel(container, context) {
  ...
  container.querySelector('#quotaResourceLink')?.addEventListener('click', () => linkUsage(container, context));
  container.querySelectorAll('[data-remove-usage]').forEach(button => button.addEventListener('click', async () => {
    if (!confirm('确定从当前定额组成中移除该资源？')) return;
    await quotaResourceService.removeUsage(button.dataset.removeUsage); await context.render();
  }));
  container.querySelectorAll('[data-refresh-usage]').forEach(button => button.addEventListener('click', async () => {
  ...
  container.querySelector('#quotaCompositionApply')?.addEventListener('click', async () => {
    if (!confirm(`应用后仅更新材料费和设备费，其他分项保持不变。总价变化 ${signedMoney(context.preview.delta.total)}，确定应用？`)) return;
    const updated = await quotaResourceService.applyComposition(context.quota.id); Object.assign(context.quota, updated); context.onApplied?.(updated); toast('资源组成已应用', 'success'); await context.render();
  });
}
