// AI 本地指令路由
import { projectRepo, quotaRepo, boqRepo } from '../data/repository.js?v=6.8';
import { pickBestQuota, categoryGuess } from '../utils/stats.js?v=6.8';
import { fmtMoney, fmt } from '../utils/dom.js?v=6.8';
import { boqService } from '../services/boqService.js?v=6.8';
import { versionService } from '../services/versionService.js?v=6.8';
import { indicatorService } from '../services/indicatorService.js?v=6.8';
  ...
  if (/复盘/.test(text) && !/(查|找|搜索|以前|历史|有没有)/.test(text)) {
    ...
    return {
      ...
      actions: [{ label: '开始复盘', onClick: async () => {
        const mod = await import('../views/experience.js?v=6.8');
