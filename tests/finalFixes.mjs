...
import { createLatestCoordinator, createLatestWorkspaceCoordinator, createSerializedKeyCoordinator, createWorkspaceRoot } from '../assets/utils/requestCoordinator.js?v=6.3';
import { buildEquipmentPackageDialog, createAtomicResourceRefresh, createLatestResourceSelection, resourceRowHtml } from '../assets/views/resources.js?v=6.3';
import { quotaResourceChoiceHtml } from '../assets/views/quotaResourceCompositionPanel.js?v=6.3';
...
async function testCoherentModuleVersionGraph() {
  ...
  for (const file of files) {
    ...
    for (const match of source.matchAll(edge)) {
      ...
      if (/^(?:\.|\/)/.test(specifier) && !specifier.endsWith('?v=6.3')) violations.push(`${file}: ${specifier}`);
    }
  }
